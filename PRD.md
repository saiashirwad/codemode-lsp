...
- **0.6.0: searchable docs + usage telemetry (search-and-execute, applied to the docs surface).** Prompted by Sentry's MCP write-up (cra.mr/a-bigger-toolbox-for-mcp): their `search_tools`/`execute_tool` pattern solves the same context-bloat problem our tool description has (three field sessions saw it truncated; 0.1.3's `help()` recovers it only by re-spending everything truncation saved). Their *execution* mechanism is rejected — code mode is demonstrably winning (one-shot field runs) — but their *discovery* mechanism is adopted for documentation: (a) `lsp.docs(query?)` (src/docs.ts), a meta-op alongside `help()`: no query → op inventory with one-liners; an op-name/keyword query → the matching ops' full signatures, the interfaces they reference (transitive), and up to two worked examples that use them; no match → the op-name list. Everything is parsed from `lsp-types.generated.ts` + `WORKED_EXAMPLES`, so `lsp-api.ts` stays the single source of truth and a new op is searchable for free. Simple substring matching, per the article's own finding that it suffices. (b) Usage telemetry (`CODEMODE_TELEMETRY` env var = JSONL file path, off by default): the operation trace — previously dropped on success — is now returned by the sandbox and, when enabled, logged per execute with ok/duration/hints/error. `docs()` calls ARE traced (unlike `help()`) because which queries models run is the gap-finding signal the article describes — field reports become data. (c) The description-diet half (shrink the shipped description to inventory + one example + "call docs()") is deliberately NOT taken yet: per this project's own methodology every description change must move the eval number, so the diet waits for an eval run comparing full vs. dieted descriptions now that `docs()` exists as the recovery path.

- **0.5.0: steer models onto the cheap path (examples > rules).** The one-shot run succeeded but spent ~150 lines and ~48s where ~15 lines and ~20s suffice; the transcript shows a consistent compliance split — everything demonstrated in a worked example (moveSymbol, checkProject, the likelyFalsePositive gate) was used verbatim, everything stated only as a prose rule (moveSymbols, lean scripts, no re-export shims) was ignored. Four levers, applied together: (a) *Delete knowledge requirements instead of documenting them* — `moveSymbols` now topo-sorts internally (post-order DFS over the to-move set's same-file deps, source order as tie-break), so "dependency order: types/helpers first" left the contract; the repointing guarantee ("EVERY importer repointed — re-export shims never needed") is now explicit in JSDoc and rules; and `getDependencyClosure(file, seeds)` owns the transitive same-file closure walk the field script hand-rolled as a 25-line getDependencies fixpoint loop — admitted under the standing bar because seed *choice* is judgment but the graph *walk* is mechanics. (b) *Make the nearest example the cheap one* — the canonical worked example is now the cluster extraction (closure → ONE moveSymbols → ONE checkProject), golden-tested like the rest. (c) *Runtime hints at the moment of waste* — the only channel proven to land mid-session is the error message (the call-hierarchy error self-corrected in one turn), so successful-but-wasteful patterns now talk back: a third single-symbol moveSymbol to the same target, or a no-op organizeImports/addMissingImports after a move, returns `WriteResult.hint` and echoes `[hint] …` into the tool's `logs` (hints are transaction-scoped, deduped, drained by the runner). (d) *Grade shape, not just outcome* — the eval runner switched to stream-json so graders see every execute script (`EvalContext.scripts`); the new `extract-session-cluster` task fails a solution that lands the right disk state via hand-splicing or >2 execute calls, and the per-task output now prints script counts. Untested hypothesis recorded for the next field run: the standing prompt's "SINGLE invocation" constraint is itself the cause of the worst third of the observed script (judgment forced into name-matching regexes before reading the file); a run without it should show the recommended discover-then-write two-call shape.
